...
public class RMF_RadialMenuElement : MonoBehaviour {
    ...
    void Start () {
        ...
        if (parentRM.useLazySelection)
            ...
            enter.callback.AddListener((eventData) => { setMenuLable(parentRM.objectName); });


            EventTrigger.Entry exit = new EventTrigger.Entry();
            exit.eventID = EventTriggerType.PointerExit;
            exit.callback.AddListener((eventData) => { setMenuLable(""); });
    ...
    public void highlightThisElement(PointerEventData p) {
        ...
        active = true;
        //setMenuLable(parentRM.objectName);
    ...
    public void setMenuLable(string l) {
        string fullLabel = label;
        
        if (l != "")
            fullLabel += "\n" + "(" + l + ")";
        
        GetComponentInChildren<Text>().text = fullLabel;
    }
